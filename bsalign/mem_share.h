...
#define num_saturate(v, l, h) (((v) > (h))? (h) : (((v) < (l))? (l) : (v)))
...
static inline void* aligned_realloc(void *buffer, size_t size, size_t cap, int base){
	...
	pad = *(q - 1);
	p = q - pad;
	p = realloc(p, cap + base);
	if(p == NULL) return NULL;
	q = (u1i*)(((u8i)(p + base)) & (~(((u8i)base) - 1)));
	if((q - p) != pad){
		memmove(q, p + pad, size);
	}
	*(q - 1) = q - p;
	return q;
}
...
static inline size_t encap_list(void **buffer, size_t e_size, size_t size, size_t cur_cap, size_t inc, int mem_zeros, size_t n_head, int aligned_base){
	void *ptr;
	size_t cap;
	...
	ptr = aligned_realloc((*buffer) - n_head * e_size, e_size * (size + n_head), e_size * (cap + n_head), aligned_base);
	if(ptr == NULL){
...
#define STEP_WISE_BUFF_SIZE	0xFFFF
